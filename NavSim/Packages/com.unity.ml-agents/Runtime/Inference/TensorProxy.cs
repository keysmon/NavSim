...
namespace Unity.MLAgents.Inference
{
    ...
    internal class TensorProxy
    {
        ...
        [NonSerialized]
        public Tensor data;
        public BackendType Device => data?.dataOnBackend?.backendType ?? BackendType.CPU;
        ...
        void Dispose()
        {
            // NavSim: the finalizer can run on a proxy whose tensor was never assigned or was
            // already released. On IL2CPP/WebGL that unchecked deref threw a NullReferenceException
            // in the finalizer, which under exceptionSupport=None became a fatal, cryptic
            // MethodAccessException that froze inference. Guard before touching the backend.
